...
def get_table_info_sample(table_list,table_info_name, actual_index,random_sample_index_list):
    ...
    for rand_sampl in random_sample_index_list:
        ...
        table_info_name_list.append(str(rand_sampl_info))
...
def cascading_retrieval(table_list,retrieval_data,query_text,actual_index,method,model=None):
    if method == "bm25":
        ...
        bm25_top_k_found_it, bm25_retrieved_index, bm25_retrieved_values,_,_ = bm25_retrieve_relevant_table(query_text, first_retrieval_corpus,first_retrieval_list_tables_info,first_retrieval_index,actual_table_info,[1,50])  
        if bm25_top_k_found_it[50]: 
            ...
            bm25_top_k_found_it, bm25_retrieved_index, bm25_retrieved_values, _,_ = bm25_retrieve_relevant_table(query_text, second_retrieval_corpus,second_retrieval_list_tables_info,second_retrieval_index,actual_table_info,[1, 5, 10, 20, 30])  
    ...
    if method == "sentence_embed":
        ...
        actual_table_info, first_retrieval_list_tables_info = get_table_info_sample(table_list,"title_col_info", actual_index,first_retrieval_index)
        first_retrieval_corpus = [table.lower() for table in first_retrieval_list_tables_info] 
        sentence_top_k_found_it, sentence_retrieved_index, sentence_retrieved_values, _, _ = sentence_embedding_retrieve_relevant_table(model,query_text, first_retrieval_corpus,first_retrieval_list_tables_info,first_retrieval_index,actual_table_info,[1,50])  
        if sentence_top_k_found_it[50]: 
            second_retrieval_index = sentence_retrieved_index[50]
            actual_table_info, second_retrieval_list_tables_info = get_table_info_sample(table_list,"title_col_table_text", actual_index,second_retrieval_index)
            second_retrieval_corpus = [table.lower() for table in second_retrieval_list_tables_info] 
            sentence_top_k_found_it, sentence_retrieved_index, sentence_retrieved_values, _, _  = sentence_embedding_retrieve_relevant_table(model,query_text, second_retrieval_corpus,second_retrieval_list_tables_info,second_retrieval_index,actual_table_info,[1, 5, 10, 20, 30])  
            for k in  [1, 5, 10, 20, 30]:
    ...
    if method == "dpr_embed":
        ...
        actual_table_info, first_retrieval_list_tables_info = get_table_info_sample(table_list,"title_col_info", actual_index,first_retrieval_index)
        first_retrieval_corpus = [table.lower() for table in first_retrieval_list_tables_info] 
        sentence_top_k_found_it, sentence_retrieved_index, sentence_retrieved_values, _, _ = dpr_retrieve_relevant_table(model,query_text, first_retrieval_corpus,first_retrieval_list_tables_info,first_retrieval_index,actual_table_info,[1,50])  
        if sentence_top_k_found_it[50]: 
            second_retrieval_index = sentence_retrieved_index[50]
            actual_table_info, second_retrieval_list_tables_info = get_table_info_sample(table_list,"title_col_table_text", actual_index,second_retrieval_index)
            second_retrieval_corpus = [table.lower() for table in second_retrieval_list_tables_info] 
            dpr_top_k_found_it, dpr_retrieved_index, dpr_retrieved_values, _, _  = dpr_retrieve_relevant_table(model,query_text, second_retrieval_corpus,second_retrieval_list_tables_info,second_retrieval_index,actual_table_info,[1, 5, 10, 20, 30])  
            for k in  [1, 5, 10, 20, 30]:
               if dpr_top_k_found_it[k]:
                    return "title_col_table_text", k, True  
            return "title_col_table_text", k, False 
        else:
            return "title_col_info", 50, False  
...
if __name__ == "__main__":
    ...
    if os.path.isfile( f'{data_path}/results/wtq/results_independent_question_{n}.csv'):
        d = pd.read_csv(f'{data_path}/results/wtq/results_independent_question_{n}.csv')
    ...
    for idx, data_point in enumerate(data):
        ...
        retrieved_data = {"bm25":{}, "sentence_embed": {},"dpr_embed":{}}
        for table_info in ["title_table-description","title_col_info", "title_col_table_text"]:
            actual_table_info, list_all_table_info = get_table_info_sample(table_list,table_info, data_point["table_index"],data_point[f"random_retrieval_sample"][1]) 
            corpus = [table.lower() for table in list_all_table_info]
            bm25_top_k_found_it, bm25_retrieved_index, bm25_retrieved_values,_,_  = bm25_retrieve_relevant_table(query_text, corpus,list_all_table_info,data_point[f"random_retrieval_sample"][1],actual_table_info,top_k_list) 
            sentence_top_k_found_it, sentence_retrieved_index, sentence_retrieved_values,_,_ = sentence_embedding_retrieve_relevant_table(sentence_model,query_text, corpus,list_all_table_info,data_point[f"random_retrieval_sample"][1],actual_table_info,top_k_list) 
            dpr_top_k_found_it, dpr_retrieved_index, dpr_retrieved_values,_,_ = dpr_retrieve_relevant_table(dpr_model,query_text, corpus,list_all_table_info,data_point[f"random_retrieval_sample"][1],actual_table_info,top_k_list) 
            ...
            retrieved_data["dpr_embed"][table_info] = (dpr_top_k_found_it[200], dpr_retrieved_index, dpr_retrieved_values) 
            ...
            for top_k in top_k_list: 
                result.append({"idx":idx,
                            "top_k":top_k,
                            "type":"independent",
                            "table_index":data_point["table_index"],
                            "table_info":table_info , 
                            "bm_25":bm25_top_k_found_it[top_k],
                            "bm_25_info":bm25_retrieved_index,
                            "sentence_embed":sentence_top_k_found_it[top_k],
                            "sentence_embed_info":sentence_retrieved_index,
                            "dpr_embed":dpr_top_k_found_it[top_k],
                            "dpr_embed_info":dpr_retrieved_index
                            })
        ...
        dpr_compare_retrievals = compare_retrievals(retrieved_data["dpr_embed"], top_k_list)
        for comparison in bm25_compare_retrievals:
            for top_k in top_k_list: 
                result.append({"idx":idx,
                        ...
                        "dpr_embed":dpr_compare_retrievals[comparison][top_k],
                        })    
                 
        ## Cascading Results 
        bm_cascade = cascading_retrieval(table_list,retrieved_data["bm25"],query_text,data_point[f"table_index"],"bm25") 
        sentence_cascade = cascading_retrieval(table_list,retrieved_data["sentence_embed"],query_text,data_point[f"table_index"],"sentence_embed",model=sentence_model) 
        dpr_cascade = cascading_retrieval(table_list,retrieved_data["dpr_embed"],query_text,data_point[f"table_index"],"dpr_embed",model=dpr_model)  
        ...
        if idx % 50 == 0:
            print(idx, data_point["table_index"], flush=True)
            print("\tBM25: \t",[bm25_retrieved_index[x] for x in [1, 5, 10]], flush=True)
            print("\tSent: \t",[sentence_retrieved_index[x] for x in [1, 5, 10]], flush=True)
            print("\tDPR : \t",[dpr_retrieved_index[x] for x in [1, 5, 10]], flush=True)
            df = pd.DataFrame(result)
            df.to_csv(f'{data_path}/results/wtq/results_independent_question_{n}.csv',index=False)
            del df
    print("Completed")
    df = pd.DataFrame(result)
    df.to_csv(f'{data_path}/results/wtq/results_independent_question_{n}.csv',index=False)
